...
if PY3:
    from .utils import mock_streams, aborts, FabricTest, assert_contains, assert_not_contains
else:
    from utils import mock_streams, aborts, FabricTest, assert_contains, assert_not_contains
...
def test_puts_with_encoding_type_none_output():
    ...
    if PY3:
        try:
            ...
            buffer = BytesIO()
            sys.stdout = TextIOWrapper(buffer, encoding=None)
            puts(s, show_prefix=False, flush=True)
            eq_(buffer.getvalue().decode('UTF-8'), s + "\n")
        finally:
